...
# Pre-built bins live in bin/prebuilt/<env>/ (generated by
# bin/regenerate_prebuilt.sh). Any env with a directory there can be flashed
# with esptool (method 2), no PlatformIO needed.
PREBUILT_ROOT="bin/prebuilt"
...
if [[ "$METHOD" == "1" ]]; then
  ...
  BINDIR="$PREBUILT_ROOT/$PIO_ENV"

  if [[ ! -f "$BINDIR/firmware.bin" ]]; then
    echo -e "${YEL}No pre-built bins found for this device ($PIO_ENV).${RST}"
    echo
    echo "Pre-built bins are available for:"
    for d in "$PREBUILT_ROOT"/*/; do
      [[ -f "$d/firmware.bin" ]] && printf "  • %s\n" "$(basename "$d")"
    done
    echo
    if $HAS_PIO; then
