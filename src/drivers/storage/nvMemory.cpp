...
bool nvMemory::saveConfig(TSettings* Settings)
{
    if (init())
    {
        ...
        // Create a JSON document (1536: WireGuard keys/endpoint/preshared key +
        // alert webhook URL would overflow the old 512-byte pool)
        ...
        json[JSON_KEY_WG_PSK] = Settings->wgPresharedKey;
        ...
        if (SPIFFS.exists(JSON_CONFIG_FILE))
        {
            ...
            if (configFile)
            {
                ...
                if (!error)
                {
                    ...
                    Settings->wgPresharedKey = json[JSON_KEY_WG_PSK] | Settings->wgPresharedKey;
